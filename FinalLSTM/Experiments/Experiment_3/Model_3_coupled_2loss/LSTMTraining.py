...
SELECTION_WEIGHT_MEDIAN = 0.5
SELECTION_WEIGHT_INTERVAL = 0.5
...
def train_epoch(
    ...
    for enc, dec, tgt in train_loader:
        ...
        # Two independent branch losses:
        #  - point branch / shared trunk from median loss
        #  - interval branch from interval loss
        ...
        weighted_score = (
            SELECTION_WEIGHT_MEDIAN * loss_median.item()
            + SELECTION_WEIGHT_INTERVAL * loss_interval.item()
        )

        epoch_loss += weighted_score * batch_size
...
def validate_epoch(model, val_loader, device, val_size, conformal_alpha=0.2):
    ...
    with torch.no_grad():
        for enc, dec, tgt in val_loader:
            ...
            loss_median = quantile_loss(q50, tgt, q=0.5)
            val_loss = (
                SELECTION_WEIGHT_MEDIAN * loss_median
                + SELECTION_WEIGHT_INTERVAL * loss_interval
            )

            batch_size = enc.size(0)
            val_loss_epoch += val_loss.item() * batch_size
    ...
    torch.save(
        {
            ...
            "optimizer_interval_state_dict": optimizer_interval.state_dict(),
            "config": vars(config),
            "epoch": epoch,
            "val_loss": val_loss,
...
def train_model(
    ...
    patience=20,
    conformal_alpha=0.2,
    model_name="Model_3 Coupled - 2 losses",
    training_variant="coupled",
):
    if training_variant != "coupled":
        raise ValueError(
            f"Model_3_coupled_2loss supports only training_variant='coupled', got '{training_variant}'."
        )
    ...
    optimizer_interval = torch.optim.AdamW(
        model.parameters(),
        lr=config.learning_rate,
        weight_decay=1e-4,
    )
    ...
    scheduler_interval = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer_interval, mode="min", factor=0.5, patience=10
    )
    ...
    if logger is not None:
        logger.info(
            ...
            f"selection_score={SELECTION_WEIGHT_MEDIAN:.2f}*median + "
            f"{SELECTION_WEIGHT_INTERVAL:.2f}*interval"
        )
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        for param_group in optimizer_interval.param_groups:
            param_group["lr"] = warmup_lr
        ...
        if epoch > 3:
            ...
            scheduler_interval.step(val_loss)
    ...
    return best_val_score, train_losses, val_losses
