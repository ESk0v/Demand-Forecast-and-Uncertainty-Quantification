...
def compute_encoder_cosine_similarity(model, loss_median, loss_interval):
    # Model_5 uses a single pinball objective, so gradient-interference cosine
    # between separate objectives is not defined.
    return 0.0


def train_epoch(model, train_loader, optimizer, device, train_size, conformal_alpha=0.2):
    model.train()
    epoch_loss = 0.0
    step_cos_sims = []
    q10_le_q50 = []
    ...
    for enc, dec, tgt in train_loader:
        ...
        # Single objective: summed pinball losses for q10/q50/q90.
        loss = (
            quantile_loss(q10, tgt, q=0.1)
            + quantile_loss(q50, tgt, q=0.5)
            + quantile_loss(q90, tgt, q=0.9)
        )
        cos_sim = 0.0
...
def plot_cosine_and_coverage(
    ...
    ax_cos.set_title(f"{model_name} — Gradient Interference (Encoder, N/A for single-loss)")
...
def train_model(
    ...
    patience=20,
    conformal_alpha=0.2,
    model_name="Model_5 Coupled - 1 pinball loss",
    training_variant="coupled",
):
    if training_variant != "coupled":
        raise ValueError(
            f"Model_5_coupled_1loss supports only training_variant='coupled', got '{training_variant}'."
        )
    ...
    if logger is not None:
        ...
        logger.info("CosSim metric is N/A for Model_5 (single objective).")
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        val_loss = validate_epoch(model, val_loader, config.device, val_size, conformal_alpha)
        ...
        if val_loss < best_val_loss:
            ...
            if logger is not None:
                logger.info(
                    ...
                    f"CosSim={train_cos_sim:.4f} (N/A)  LR={current_lr:.2e}  [best — saving checkpoint]"
                )
            ...
            if logger is not None:
                logger.info(
                    f"Epoch {epoch}: Train={train_loss:.4f}  Val={val_loss:.4f}  "
                    f"CosSim={train_cos_sim:.4f} (N/A)  LR={current_lr:.2e}  "
