...
def train_epoch(model, train_loader, optimizer_point, optimizer_spread, device, train_size, conformal_alpha=0.2):
    ...
    for enc, dec, tgt in train_loader:
        ...
        g1_tensors = [p.grad.flatten() for p in model.encoderLstm.parameters() if p.grad is not None]
        g1 = torch.cat(g1_tensors) if g1_tensors else None

        # Zero the gradients so we get PURE interval loss gradients for g2
        optimizer_point.zero_grad()
        optimizer_spread.zero_grad()

        loss_interval.backward(retain_graph=True)

        g2_tensors = [p.grad.flatten() for p in model.encoderLstm.parameters() if p.grad is not None]
        g2 = torch.cat(g2_tensors) if g2_tensors else None
        ...
        # In order to properly update the weights, we must restore the combined point-forecast gradients 
        # (since we artificially zeroed them just to inspect g2 purely)
        # However, because of gradient interference, a simple `loss_median.backward()` without zeroing might be safer 
        # but let's re-run loss_median.backward() to accumulate correctly
        loss_median.backward(retain_graph=True)
    ...
    avg_cos_sim = np.mean(epoch_cos_sims) if len(epoch_cos_sims) > 0 else 0.0

    print("q10<=q50 avg:", np.mean(q10_le_q50))
    print("q50<=q90 avg:", np.mean(q50_le_q90))
    if len(epoch_cos_sims) > 0:
...
def save_checkpoint(model, optimizer_point, optimizer_spread, config, epoch, val_loss,
                    train_losses, val_losses, train_cos_sims, model_save_path):
    torch.save({
        ...
        'train_cos_sims':             train_cos_sims.copy(),
    }, model_save_path)
...
def plot_cosine_similarity(train_cos_sims, save_path):
    """Plot gradient cosine similarity over epochs."""
    fig, ax = plt.subplots(figsize=(10, 5))
    epochs_range = range(1, len(train_cos_sims) + 1)
    ax.plot(epochs_range, train_cos_sims, label="Gradient Cosine Sim", linewidth=1.5, marker='o', markersize=2, color='purple')
    ax.set_xlabel("Epoch", fontsize=12)
    ax.set_ylabel("Cosine Similarity", fontsize=12)
    ax.set_title("Encoder LSTM Gradient Cosine Similarity (Median vs Interval Loss)", fontsize=14)
    ax.legend(fontsize=10)
    ax.grid(True, alpha=0.3)
    ax.set_ylim(-1.1, 1.1)
...
def train_model(config, train_loader, val_loader, cal_loader,
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        train_cos_sims.append(train_cos_sim)
    ...
    checkpoint['train_cos_sims'] = train_cos_sims
    ...
    cossim_plot_path   = os.path.join(plot_dir, "gradient_cosine_similarity.png")
    ...
    plot_cosine_similarity(train_cos_sims, cossim_plot_path)

    if logger is not None:
        logger.info(f"Loss curve saved to {loss_plot_path}")
        logger.info(f"Coverage plot saved to {coverage_plot_path}")
        logger.info(f"Cosine Similarity plot saved to {cossim_plot_path}")
