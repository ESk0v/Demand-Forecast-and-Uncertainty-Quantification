...
def train_epoch(model, train_loader, optimizer_point, optimizer_spread, device, train_size, conformal_alpha=0.2):
    ...
    for enc, dec, tgt in train_loader:
        ...
        # --- Gradient interference check: record gradients before spread backward ---
        ...
        loss_spread = loss_interval
        loss_spread.backward()

        # --- Gradient interference check: compare gradients after spread backward ---
        if enc_grad_before is not None and model.encoderLstm.weight_ih_l0.grad is not None:
            if not torch.allclose(enc_grad_before, model.encoderLstm.weight_ih_l0.grad):
                diff = (model.encoderLstm.weight_ih_l0.grad - enc_grad_before).abs()
                print(f"WARNING: Gradient interference in encoderLstm! Max diff: {diff.max().item():.6e}, Mean diff: {diff.mean().item():.6e}, Changed param count (>1e-7): {(diff > 1e-7).sum().item()}/{diff.numel()}")
        if dec_grad_before is not None and model.decoderLstm.weight_ih_l0.grad is not None:
            if not torch.allclose(dec_grad_before, model.decoderLstm.weight_ih_l0.grad):
                diff = (model.decoderLstm.weight_ih_l0.grad - dec_grad_before).abs()
                print(f"WARNING: Gradient interference in decoderLstm! Max diff: {diff.max().item():.6e}, Mean diff: {diff.mean().item():.6e}, Changed param count (>1e-7): {(diff > 1e-7).sum().item()}/{diff.numel()}")
        # --------------------------------------------------------------------------
