...
class LSTMForecast(nn.Module):
    def __init__(self, config: Config):
        ...
        self.uncertainty_hidden_proj   = nn.Linear(config.hidden_size, config.hidden_size)
    ...
    def _init_weights(self):
        for name, param in self.named_parameters():
            ...
            elif name in ('uncertainty_cell_proj.weight', 'uncertainty_hidden_proj.weight'):
                nn.init.eye_(param.data)
            elif name in ('uncertainty_cell_proj.bias', 'uncertainty_hidden_proj.weight'):
                nn.init.constant_(param.data, 0)
