...
def train_epoch_spread(model, train_loader, optimizer, device, train_size, conformal_alpha=0.2):
    """
    One pass training the uncertainty LSTM + projections + ramp only.
    The encoder and decoder are frozen.

    The crossing penalty keeps q10 ≤ q50 ≤ q90 throughout training.
    """
    model.train()
    epoch_loss   = 0.0
    q10_le_q50   = []
    ...
    for enc, dec, tgt in train_loader:
        ...
        loss = loss_interval + crossing_penalty
        loss.backward()
...
# ─── Conformal (external, optional) ──────────────────────────────────────────

def apply_conformal(q10, q90, u_alpha):
    """
    Apply saved conformal calibration offsets to new predictions.
    Kept here for inference pipelines that still use it externally.

    u_alpha may be a scalar or a per-horizon vector.
    """
    is_tensor = isinstance(q10, torch.Tensor)
    if is_tensor:
        device = q10.device
