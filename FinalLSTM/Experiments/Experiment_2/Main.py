"""
AblationStudy_UncertaintyDecoder/main.py
─────────────────────────────────────────
Ablation: UncertaintyDecoder vs. Direct Output Head

  Model A  —  Baseline architecture:
               encoder → decoderLSTM (q50) + uncertaintyDecoderLSTM (spread)
               → q10 = q50 - spread_lo * ramp
               → q90 = q50 + spread_hi * ramp

  Model B  —  Ablated architecture:
               encoder → decoderLSTM with three output heads
               → q10, q50, q90 predicted directly (no uncertainty decoder, no ramp)

Both models receive identical inputs (same dataset, same splits, same seed).
Run Plot.py after training to generate comparison figures.
"""
...
#
#   module_dir      : subfolder containing LSTMModel.py + LSTMTraining.py
...
MODEL_CONFIGS = [
    ...
    dict(
        ...
        conformal_alpha = 0.10,   # same target for a fair comparison
        epochs          = 50,
        patience        = 5,
    ),
]
...
def train_one_model(cfg: dict):
    ...
    subfolder, trains the model, and saves the checkpoint to disk.

    The dynamic import pattern means Model_Baseline and Model_DirectHead
    can each carry their own LSTMModel.py without namespace collisions.
    """
    import importlib
    ...
    # ── Isolate module imports so each model gets a fresh load ────────────────
    ...
    if not os.path.isfile(dataset_path):
        raise FileNotFoundError(
            ...
            f"Place your dataset.pt inside "
            f"{os.path.join(abs_module_dir, 'data')}"
        )
    ...
    logger.success(
        f"[{cfg['name']}] Checkpoint saved → {model_save_path}"
    )

    # ── Clean up so the next model gets a fresh import ────────────────────────
    if abs_module_dir in sys.path:
        sys.path.remove(abs_module_dir)
    for mod_name in ("LSTMTraining", "LSTMModel"):
        if mod_name in sys.modules:
...
def main():
    ...
    logger.success(
        "All models trained. Run Plot.py to generate comparison figures."
    )
